...
void SVMtrain(Mat &trainMat,vector<int> &trainLabels, Mat &testResponse,Mat &testMat,float gamma1,int RBF_LINEAR){
#ifdef USE_OPENCV_2
    CvSVMParams params;
    ...
    svm->predict(testMat, testResponse);
    clkFinish = clock();//
    cout << "Time taken is " << (clkFinish - clkStart) << endl;//   
    getSVMParams(svm);
#endif
}
...
int main(){
    ...
    int RBF_LINEAR=0; 
    float gamma1=1.0;
    cout << "RBF or Linear(1 for RBF and 0 for Linear) : " << endl;
    cin >> RBF_LINEAR;
    if(RBF_LINEAR==1){
        cout << "gamma : " << endl;
        cin >> gamma1;
    ...
    Mat testResponse;
    SVMtrain(trainMat,trainLabels,testResponse,testMat,gamma1,RBF_LINEAR); 
