...
static int detectAndDraw(const HOGDescriptor &hog, Mat &img)
{
    vector<Rect> found, found_filtered;
    //double t = (double) getTickCount(); // Changed...
    //double scalFac = 1.05 ; // Added
    // Run the detector with default parameters. to get a higher hit-rate
    // (and more false alarms, respectively), decrease the hitThreshold and
    // groupThreshold (set groupThreshold to 0 to turn off the grouping completely).
    hog.detectMultiScale(img, found, 0, Size(winStride,winStride), Size(padding,padding), scalFac, 2);
    //Added: hog.detectMultiScale(Mat img, MatOfRect foundLocations, MatOfDouble foundWeights, 
    //                            Size winStride, Size padding, double scale, double finalThreshold)
    // commented ....t = (double) getTickCount() - t;
    //COMMENTED...cout << "detection time = " << (t*1000./cv::getTickFrequency()) << " ms" << endl;
    //cout << "Detected Recs =" << found.size() << "  "<< endl;
...
int main(int argc, char** argv)
{
    ...
    cout << "Scaling Factor = " << (scalFac) <<  endl;
    cout << "Win Stride = " << (winStride) <<  endl;
...
t = (double) getTickCount() - t;
    ...
    double acc = (corCount*100.0)/totalFrames ;
